...
export default function MinistryStatsPage() {
  const t = useTranslations();
  const { can } = useAuth();
  const [items, setItems] = useState<{ id: string; name: string; members: number }[]>([]);

  useEffect(() => {
    entityMemberStats("ministries").then(setItems);
  }, []);

  if (!can("view_ministry_stats")) return <EmptyState message={t("auth.noAccess")} />;

  return (
    <div>
      <PageHeader title={t("nav.ministryStats")} />
      <Table>
