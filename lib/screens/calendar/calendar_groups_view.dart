...
class CalendarGroupsView extends StatelessWidget {
  final DateTime selectedDate;
  final List<GroupEvent> events;

  const CalendarGroupsView({
    super.key,
      ...
      return Center(
        child: Column(
          ...
          children: [
            ...
            Text(
              AppLocalizations.of(context)!.noGroupEventsScheduled,
              style: TextStyle(
                fontSize: 16,
  ...
  Widget _buildEventCard(BuildContext context, GroupEvent event) {
    return Card(
              ...
              child: Column(
                ...
                children: [
                  ...
                  FutureBuilder<DocumentSnapshot>(
                    future: event.groupId.get(),
                    builder: (context, snapshot) {
                      final groupName = snapshot.hasData && snapshot.data!.exists
                          ? (snapshot.data!.data() as Map<String, dynamic>)['name'] ?? 'Grupo'
                          : 'Grupo';
  ...
  void _addReminder(BuildContext context, GroupEvent event) async {
    try {
      // Obtener el nombre del grupo
      final groupDoc = await event.groupId.get();
      final groupName = groupDoc.exists 
          ? (groupDoc.data() as Map<String, dynamic>)['name'] ?? 'Grupo'
          : 'Grupo';
  ...
  void _toggleAttendance(BuildContext context, GroupEvent event, bool attending) async {
    try {
