...
class _MinistryHistoryTabState extends State<MinistryHistoryTab> {
  ...
  Map<String, Map<String, List<Map<String, dynamic>>>> _ministriesData = {};
  ...
  void _applyDateFilter() {
    ...
    for (final ministryId in _ministriesData.keys) {
      final ministryData = _ministriesData[ministryId]!;
      
      // Recorrer todos los tipos de registros
      for (final status in ['pending', 'accepted', 'rejected', 'exited']) {
        for (final member in ministryData[status] ?? []) {
  ...
  void _updateCounters() {
    ...
    for (final ministry in widget.ministries) {
      ...
      if (!_ministryDetailedStats.containsKey(ministryId)) {
        _ministryDetailedStats[ministryId] = {
      ...
      if (ministryData != null) {
        // Contadores básicos
        final pendingCount = ministryData['pending']?.length ?? 0;
        final acceptedCount = ministryData['accepted']?.length ?? 0;
        final rejectedCount = ministryData['rejected']?.length ?? 0;
        final exitedCount = ministryData['exited']?.length ?? 0;
  ...
  void _sortMinistries() {
    ...
    // Como no podemos modificar widget.ministries directamente (es inmutable),
    // vamos a mantener el orden en una lista separada y usar esa para el render
  ...
  Widget _buildDateFilteredView() {
    ...
    final Map<String, List<Map<String, dynamic>>> recordsByMinistry = {};
    
    for (final record in _filteredByDateMembers) {
      final ministryId = record['ministryId'] as String;
      
      if (!recordsByMinistry.containsKey(ministryId)) {
        recordsByMinistry[ministryId] = [];
      }
      
      recordsByMinistry[ministryId]!.add(record);
    }
    
    // Crear lista de ministerios ordenada
    final List<String> sortedMinistryIds = recordsByMinistry.keys.toList();
            ...
            child: ExpansionTile(
              ...
              children: [
                ListView.builder(
                  ...
                  itemCount: ministryRecords.length,
                  itemBuilder: (context, recordIndex) {
                    final memberData = ministryRecords[recordIndex];
  ...
  Widget _buildNormalView() {
    return ListView.builder(
      ...
      itemBuilder: (context, index) {
        ...
        final ministryId = ministry.id;
        final ministryAllData = _ministriesData[ministryId] ?? {};
        
        // Obtener el filtro seleccionado para este ministerio o usar el predeterminado (todos)
        final selectedFilter = _ministryFilters[ministryId];
        ...
        final counts = _ministryCounts[ministryId] ?? {
          'pending': 0, 'accepted': 0, 'rejected': 0, 'exited': 0, 'total': 0
        };
        
        // Obtener estadísticas detalladas
        final stats = _ministryDetailedStats[ministryId] ?? {
          'members_current': 0,
          'total_entries': 0,
          'added_direct': 0,
          'added_request': 0,
                              ...
                              child: Row(
                                children: [
                                  _buildFilterChip(
                                    ...
                                    onSelected: (selected) {
                                      setState(() {
                                        _ministryFilters[ministryId] = null;
                                      });
                                    },
                                  ),
                                  const SizedBox(width: 8),
                                  _buildFilterChip(
                                    ...
                                    onSelected: (selected) {
                                      setState(() {
                                        _ministryFilters[ministryId] = selected ? 'pending' : null;
                                      });
                                    },
                                  ),
                                  const SizedBox(width: 8),
                                  _buildFilterChip(
                                    ...
                                    onSelected: (selected) {
                                      setState(() {
                                        _ministryFilters[ministryId] = selected ? 'accepted' : null;
                                      });
                                    },
                                  ),
                                  const SizedBox(width: 8),
                                  _buildFilterChip(
                                    ...
                                    onSelected: (selected) {
                                      setState(() {
                                        _ministryFilters[ministryId] = selected ? 'rejected' : null;
                                      });
                                    },
                                  ),
                                  const SizedBox(width: 8),
                                  _buildFilterChip(
                                    ...
                                    onSelected: (selected) {
                                      setState(() {
                                        _ministryFilters[ministryId] = selected ? 'exited' : null;
                                      });
                                    },
                                  ),
                                ],
