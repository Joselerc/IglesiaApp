...
class _GroupChatScreenState extends State<GroupChatScreen> {
  ...
  void _checkAdminStatus() {
    ...
    if (currentUser != null) {
      setState(() {
        _isAdmin = widget.group.adminIds.contains(currentUser.uid);
      });
    }
  }
  ...
  Future<void> _showImagePreviewDialog(File file, String fileName, String fileExtension) async {
    ...
    if (shouldSend == true) {
      ...
      final storagePath = 'group_chats/${widget.group.id}/$timestamp-$fileName';
  ...
  Widget _buildFilePreview(String fileUrl, String fileType, String fileName, [String? messageId]) {
    ...
    switch (fileType) {
      ...
      case 'pdf':
        return GestureDetector(
          onTap: isValidUrl ? () => _openFileForDownload(fileUrl, fileName) : null,
          child: Container(
    ...
    return Scaffold(
      appBar: AppBar(
        ...
        title: GestureDetector(
          ...
          child: Row(
            children: [
              // Imagen o avatar del grupo - solo verificar si está vacío
      ...
      if (_soundRecorder.isRecording) {
        ...
        if (finalDuration.inSeconds > 1 && path != null) {
          ...
          try {
            ...
            final storagePath = 'group_chats/${widget.group.id}/$fileName';
  ...
  // Añade este método para obtener los nombres de los miembros del grupo
  Future<List<String>> _getGroupMembersNames() async {
    try {
      ...
      List<dynamic> membersRefs = groupData['members'] ?? [];
